...
class SaiThriftVppUdpPktTest(SaiHelperSimplified):
    """ Test saithrift vnet outbound towards vpp"""

    def setUp(self):
        super(SaiThriftVppUdpPktTest, self).setUp()
        self.switch_id = 5
    ...
    def configureVnet(self):
        ...
        # Configure underlay route
        pa_prefix = sai_thrift_ip_prefix_t(addr_family=self.sai_ip_addr_family,
                                           addr=sai_thrift_ip_addr_t(ip4=self.dst_pa_ip),
                                           mask=sai_thrift_ip_addr_t(ip4="255.255.0.0"))
        self.pa_route_entry = sai_thrift_route_entry_t(switch_id=self.switch_id, destination=pa_prefix)
        status = sai_thrift_create_route_entry(self.client, self.pa_route_entry,
                                               packet_action=SAI_PACKET_ACTION_FORWARD, next_hop_id=self.vpp_port)
        assert(status == SAI_STATUS_SUCCESS)
    ...
    def trafficTest(self):
        ...
        print("\tVerifying packet in vpp port...")
        verify_packet(self, self.pkt_exp, self.vpp_port)
        print ("SaiThriftVppUdpPktTest OK")
    ...
    def tearDown(self):
        ...
        try:
            ...
            super(SaiThriftVppUdpPktTest, self).tearDown()
