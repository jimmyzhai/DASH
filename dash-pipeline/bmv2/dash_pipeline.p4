...
#define ACL_GROUPS_COPY_TO_META(prefix) \
   ...
   meta.stage5_dash_acl_group_id = ## prefix ##_stage5_dash_acl_group_id;
    ...
    action set_eni_attrs(bit<32> cps,
                         ...
                         @SaiVal[type="sai_uint16_t"] bit<16> vnet_id,
    ...
    table pa_validation {
        key = {
            meta.eni.vnet_id: exact @SaiVal[type = "sai_uint16_t"];
    ...
    action set_pipeline(@SaiVal[type="sai_object_id_t"] dash_oid_t eni_id) {
        meta.eni_id = eni_id;
    }
